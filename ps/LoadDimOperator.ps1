<# 
Script:  LoadDimOperator
Purpose: Loads new and updated rows to DimOperator
Author:  Trevor Howe
Date:    14 August 2016
#>
...
Try 
{
    ...
    $objects = @("regional_area", "district", "province", "authority","municipality","user","centre")
    ...
    if ($loadflag -eq 0) 
    {
        ...
        # Load Dim
        $SqlCmd.CommandText = "prcLoadDimEBATRolePlayerUser"
        $SqlCmd.Parameters.Clear()
        $SqlCmd.Parameters.AddWithValue("@AuditKey",$AuditKey)
        $SqlCmd.ExecuteNonQuery()
