<# 
Script:  LoadDimOfficer
Purpose: Loads new and updated rows to DimOfficer
Author:  Trevor Howe
Date:    14 August 2016
#>
...
Try 
{
    ...
    $objects = @("officer","rank","authority","province","station","municipality","district")
    ...
    if ($loadflag -eq 0) 
    {
        ...
        # Load DimEBATRolePlayer
        $SqlCmd.CommandText = "dbo.prcLoadDimEBATRolePlayerOfficer"
        $SqlCmd.Parameters.Clear()
        $SqlCmd.Parameters.AddWithValue("@AuditKey",$AuditKey)
        $SqlCmd.Parameters.AddWithValue("@DeltaLogKey",$DeltaLogKey)
        $SqlCmd.ExecuteNonQuery()
