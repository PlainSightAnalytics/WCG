<# 
Script:  LoadDimImpoundInstruction
Purpose: Loads new rows to DimImpoundInstruction from Impound (pnd)
Author:  Trevor Howe
Date:    04-02-2019
#>
...
Try 
{
    ...
    # Declare Variables
    $tablename = "DimImpoundInstruction"
    $clientname = "WCG"
    $schemaname = "pnd"
    $objectname = "impound_instruction"
    $auditkey = -1
    $ExecutionLogKey = -1
    $UnloadedDeltas = @()
    ...
    if ($UnloadedDeltas.Count -gt 0) 
    {
        ...
        foreach ($DeltaLogKey in $UnloadedDeltas) {
            ...
            $SqlCmd.CommandText = "dbo.prcLoadDimImpoundInstruction"
            $SqlCmd.Parameters.Clear()
