<# 
Script:  LoadFactAlerts
Purpose: Loads new rows to DimVehicle
Author:  Trevor Howe
Date:    6 August 2016
#>
...
Try 
{
    ...
    # Declare Variables
    $tablename = "DimVehicle"
    $clientname = "WCG"
    $schemaname = "cle"
    $object = "VehicleEnquiryResponsesHistory"
    $auditkey = -1
    $ExecutionLogKey = 0
    $UnloadedDeltas = @()
    ...
    $SqlCmd.Parameters.AddWithValue("@ReturnDeltas",234)
    ...
    foreach ($DeltaLogKey in $UnloadedDeltas) {
        ...
        $SqlCmd.CommandText = "dbo.prcLoad" + $TableName + "History"
        ...
        $SqlCmd.CommandText = "dbo.prcLoadDimVehicleTypeHistory"
        $SqlCmd.Parameters.AddWithValue("@DeltaLogKey",$DeltaLogKey)
        $SqlCmd.Parameters.AddWithValue("@AuditKey",$AuditKey)
        $SqlCmd.CommandTimeout = 0
        $SqlCmd.ExecuteNonQuery()
