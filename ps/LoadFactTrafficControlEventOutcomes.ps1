<# 
Script:  LoadFactTrafficControlEventOutcomes
Purpose: Loads new rows to FactTrafficControlEventOutcomes
Author:  Trevor Howe
Date:    19 May 2018
...
Try 
{
    ...
    # Declare Variables
    $tablename = "FactTrafficControlEventOutcomes"
    $clientname = "WCG"
    $schemaname = "itis"
    $objectname = "violation_charge"
    $auditkey = -1
    $ExecutionLogKey = -1
    $UnloadedDeltas = @()
    ...
    if ($UnloadedDeltas.Count -gt 0) 
    {
        ...
        $SqlCmd.CommandText = "dbo.prcUpdateFactTrafficControlEventOutcomesOperation"
        $SqlCmd.Parameters.Clear()
        $SqlCmd.Parameters.AddWithValue("@AuditKey",$AuditKey)
        $SqlCmd.CommandTimeout = 0
        $SqlCmd.ExecuteNonQuery()
