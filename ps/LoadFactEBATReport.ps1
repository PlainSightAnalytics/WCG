<# 
Script:  LoadFactEBATReport
Purpose: Loads new rows to FactEBATReport
Author:  Trevor Howe
Date:    15 August 2016
#>
...
Try 
{
    ...
    # Declare Variables
    $tablename = "FactEBATReport"
    $clientname = "WCG"
    $schemaname = "ebat"
    $objectname = "ebat_report"
    $auditkey = -1
    $ExecutionLogKey = -1
    $UnloadedDeltas = @()
    ...
    if ($UnloadedDeltas.Count -gt 0) 
    {
        ...
        foreach ($DeltaLogKey in $UnloadedDeltas) {
            ...
            # Load EBAT Event Duration Fact
            $SqlCmd.CommandText = "prcLoadFactEBATEventDuration"
            $SqlCmd.Parameters.Clear()
            $SqlCmd.Parameters.AddWithValue("@DeltaLogKey",$DeltaLogKey)
            $SqlCmd.Parameters.AddWithValue("@AuditKey",$AuditKey)
            $SqlCmd.ExecuteNonQuery()
