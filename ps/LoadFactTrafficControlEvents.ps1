<# 
Script:  LoadFactTrafficControlEvents
Purpose: Loads new rows to FactTrafficControlEvents
Author:  Trevor Howe
Date:    20 May 2018
...
Try 
{
    ...
    # Declare Variables
    $tablename = "FactTrafficControlEvents"
    $clientname = "WCG"
    $schemaname = "itis"
    $objectname = "event"
    $auditkey = -1
    ...
    if ($UnloadedDeltas.Count -gt 0) 
    {
        ...
        foreach ($DeltaLogKey in $UnloadedDeltas) {
            ...
            $SqlCmd.CommandText = "dbo.prcLoadDimTrafficControlEvent"
            $SqlCmd.Parameters.Clear()
        ...
        $SqlCmd.CommandText = "dbo.prcUpdateFactTrafficControlEventsOperation"
        $SqlCmd.Parameters.Clear()
        $SqlCmd.Parameters.AddWithValue("@AuditKey",$AuditKey)
        $SqlCmd.CommandTimeout = 0
        $SqlCmd.ExecuteNonQuery()
