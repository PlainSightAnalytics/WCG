<# 
Script:  LoadStageAlertsSlice.ps1
...
Try 
{
    ...
    $DestinationSQLCmd.CommandText = "dbo.prcGetHighWaterMarkAlertsSlice"
    $DestinationSQLCmd.Parameters.Clear()
    $DestinationSQLCmd.Parameters.AddWithValue("@ClientName",$clientname)
    $DestinationSQLCmd.Parameters.AddWithValue("@SchemaName",$schemaname)
    ...
    if ($RowCountSource -gt 0) {
        ...
        $SourceSQLCmd.CommandText += "AlertRecordId,VRN,AlertDateTime,AlertType,AlertSubType,Latitude,Longitude,SightingId,AverageKmh,DeviceId,Lane,LicenceNumber,VehicleCategory,VehicleUsage "
        ...
        $bulkCopy.DestinationTableName = $schemaname + "." + $destinationobject
