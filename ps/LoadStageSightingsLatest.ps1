<# 
Script:  LoadStageSightingsLatest.ps1
Purpose: Loads Sightings for the last 48 hours from CLE
Author:  Trevor Howe
Date:    2019-03-06
#>
...
Try 
{
    ...
    $pa_rowcountstage = 0 
    $pa_deltalogkey = 1
    $pa_previousdeltalogkey = 0
    $pa_auditkey = -1
    $pa_PreviousHighWaterMark = ""
    $pa_HighWaterMark = 0
    $pa_clientname = "WCG"
    $pa_schemaname = "cle"
    $pa_object = "SightingsLatest"
    ...
    $pa_DestinationConnection.ConnectionString = "Server= $wcgserver ;Database= $wcgstagedb ;Integrated Security=True;Timeout = 0;"
    ...
    $pa_DestinationSQLCmd.CommandText ="cle.prcLoadSightingsLatest"
    $pa_DestinationSQLCmd.Parameters.Clear()
    $pa_DestinationSQLCmd.Parameters.AddWithValue("@DaysPrevious",2)
    $pa_OutputParm = $pa_DestinationSQLCmd.Parameters.AddWithValue("@LastSightingRecordId",$pa_PreviousHighWaterMark)
    $pa_OutputParm.Direction = 'Output'
	...
	$pa_SourceSQLCmd.CommandText += " SightingRecordId,ProviderId,CameraId,Timestamp,LaneID,XCoord,YCoord,VRN"
    $pa_SourceSQLCmd.CommandText += " FROM Sightings WITH (NOLOCK) WHERE SightingRecordId > " + $pa_PreviousHighWaterMark
    $pa_SourceSQLCmd.CommandTimeout = 0 
    ...
    $pa_bulkCopy.DestinationTableName = $pa_schemaname + ".SightingsLatest"
