<# 
Script:  LoadFactShiftOutcomes
Purpose: Loads new and updated rows to FactShiftOutcomes
Author:  Trevor Howe
Date:    28-10-2018
#>
...
Try 
{
    ...
    # Declare Variables
    $tablename = "FactShiftOutcomes"
    $clientname = "WCG"
    $schemaname = "itis"
    $objectname = "critical_outcome"
    $auditkey = -1
    $ExecutionLogKey = -1
    $UnloadedDeltas = @()
    ...
    if ($UnloadedDeltas.Count -gt 0) 
    {
        ...
        foreach ($DeltaLogKey in $UnloadedDeltas) {
            ...
            # Load Dimension
            $SqlCmd.CommandText = "dbo.prcLoadDimCriticalOutcomeType"
            $SqlCmd.Parameters.Clear()
