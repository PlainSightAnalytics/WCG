<# 
Script:  LoadStageSightingsBoot.ps1
Purpose: Loads history data from Sightings CLE
Author:  Trevor Howe
...
$scriptname = "LoadStageSightingsBoot.ps1"       
...
# Get Unloaded Deltas
$SqlCmd.CommandText = "SELECT CONCAT(FromId,';',ToId,';',RowCountSource) AS DeltaString FROM WCG_Stage.dbo.SightingsDeltas ORDER BY LogDate"
...
foreach ($DeltaString in $UnloadedDeltas)
{
    ...
    if ($RowCountSource -gt 0) {
        ...
        $SourceSQLCmd.CommandText += " FROM Sightings WITH (NOLOCK) WHERE SightingRecordId between " + $PreviousHighWaterMark + " AND " + $HighWaterMark
