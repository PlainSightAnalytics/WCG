<# 
Script:  LoadStageEBATReport.ps1
Purpose: Loads ebat.ebat_report stage table from JSON data
Author:  Trevor Howe
Date:    15 August 2016
#>
...
Try 
{
    ...
    $HighwaterDateTimeString = $HighWaterDateTime.AddSeconds(7201).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")

    # Build Latest Update uri
    $lastupdatedateuri = "$BaseURI/$username/objects/$object.$objecttype`?sort[_updated_at]=desc&limit=1"
    
    # Build Count uri
    $counturi = "$BaseURI/$username/objects/$object/count.$objecttype`?q[_updated_at.gt]=$HighWaterDateTimeString"
    ...
    if ($rowcountsource -gt 0) {
        ...
        $waitseconds =5
        $limit = 1000
        $datauri = "$BaseURI/$username/objects/$object.$objecttype`?q[_updated_at.gt]=$HighWaterDateTimeString"
        ...
        foreach ($file in $files){
            ...
            $SqlCmd.Parameters.AddWithValue("@AuditKey",$AuditKey)
            $SqlCmd.Parameters.AddWithValue("@TruncateFlag",$truncatetable)
            $SqlCmd.ExecuteNonQuery()
            $truncatetable = 0
