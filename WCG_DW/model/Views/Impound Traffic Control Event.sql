...
CREATE VIEW [model].[Impound Traffic Control Event] 
...
FROM WCG_DW.dbo.DimTrafficControlEvent a WITH (NOLOCK)
WHERE EXISTS (
	SELECT TrafficControlEventKey 
	FROM model.[_Impound Requests] b WITH (NOLOCK) 
	WHERE 
		a.TrafficControlEventKey = b.TrafficControlEventKey
)
