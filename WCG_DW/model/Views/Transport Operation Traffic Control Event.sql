...
CREATE VIEW [model].[Transport Operation Traffic Control Event] AS 
------------------------------------------------------------------------------------------
-- Author               :   Trevor Howe
-- Date Created         :   04-10-2019
-- Reason               :   Traffic Control Events related to Transport Operation Events
------------------------------------------------------------------------------------------
-- Modified By          :	
-- Modified On          :	
-- Reason               :	
...
SELECT 
	...
	,[Travel Direction]
	,[Trip Type]
FROM WCG_DW.model.[Traffic Control Event] t WITH (NOLOCK)
WHERE 
	EXISTS (
		SELECT TrafficControlEventKey
		FROM model.[_Transport Operation Events] e WITH (NOLOCK)
		WHERE t.TrafficControlEventKey = e.TrafficControlEventKey
	) 
	OR 
	EXISTS (
		SELECT TrafficControlEventKey
