...
CREATE VIEW [dbo].[LoadFactImpoundViolationCharges] 
...
SELECT 
	...
	,ISNULL(d9.ViolationChargeKey,-1)		AS ViolationChargeKey
	,tfm.UniqueID							AS UniqueID
	,tfm.[DeltaLogKey]						AS DeltaLogKey
FROM [WCG_Stage].[pnd].[transformFactImpoundViolationCharges] tfm WITH (NOLOCK)
...
LEFT JOIN [WCG_DW].[dbo].[DimViolationCharge] d9 WITH (NOLOCK) ON tfm.ViolationCodeID = d9.ChargeGUID
